Scriptname foc:FOC_FusionGenActivator extends ObjectReference
...
Sound Property GeneratorON Auto Const
{Sound to play when turning generator on}
ObjectReference Property CoreMarker Auto Const
{Marker so we know where we're putting the fusion core item}
Keyword Property BlockPlayerActivation Auto Const
{Just so we don't see the generator activation prompt all the time}
Message Property InsertCoreMessage Auto Const
...
Event OnActivate(ObjectReference akAR)
    int Menu = InsertCoreMessage.Show()
    if Menu != 0 ;'Do Nothing'
        Objectreference FusionCore = akAR.DropObject(FusionCoreBaseObject)
        FusionCore.SetMotionType(Motion_Keyframed)
        FusionCore.MoveTo(CoreMARKER)
        FusionCore.SetScale(1.40)
        MyFusionCore = FusionCore
        bCoreTaken = False
        fOffTimerSoFar = 0.00
        registerForRemoteEvent(myFusionCore, "OnActivate")
 
        GeneratorON.play(Self)
        linkedlight.enable()
        ...
        WaitRandomSlice(2)
        linkedlight.enable()
        TroubleMarker.enable()
        fOffTimerSoFar = 0.00
        playAnimation("End")
        playAnimation("Reset")
 
        GeneratorLoopMarkerA.enable()
        if GeneratorLoopMarkerB
            ; only enable secondary marker if on exists.
            GeneratorLoopMarkerB.enable()
        endif
    EndIf
